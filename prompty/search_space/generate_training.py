...
class BestShotsSelector:
    ...
    def _get_embeddings(self) -> None:
        ...
        if not self._embeddings:
            ...
            self._embeddings = model.encode(self.examples, normalize_embeddings=True)  # using normalise to make cosine sim = dot product
    ...
    def min_max_diverse_subset(self, k: int = 10, seed: int = 9425) -> List[str]:
        ...
        """
        assert k <= len(self._examples), "k must be less than or equal to number of sentences"
        ...
        while len(selected_indices) < k:
            dists = cosine_distances(self._embeddings, self._embeddings[selected_indices])
    ...
    def query_similarity_select(self, k: int, query: Optional[str] = None) -> Optional[List[str]]:
        """
